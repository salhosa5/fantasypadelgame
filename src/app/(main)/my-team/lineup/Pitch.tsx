...
type Props = {
  ...
  opponentMap?: Record<number, string>; // NEW
};
...
export default function Pitch({
  ...
  const PitchRow = ({ ids }: { ids: number[] }) => (
    <div className="flex flex-wrap justify-center gap-3">
      {ids.map((id) => (
        <PlayerCard key={id} id={id} side="S" />
      ))}
    </div>
  );
  const BenchRow = ({ ids }: { ids: number[] }) => (
    <div className="flex flex-wrap justify-center gap-3">
      {ids.map((id) => (
        <PlayerCard key={id} id={id} side="B" />
      ))}
    </div>
  );
  ...
  const PlayerCard = ({ id, side }: { id: number; side: "S" | "B" }) => {
    ...
    const opp =
      (p.team?.id != null && opponentMap && opponentMap[p.team.id]) || null;

    return (
      <div
        onClick={onClick}
        className={`relative w-[128px] min-w-[128px] cursor-pointer select-none rounded-xl border bg-white/90 px-2 py-2 shadow-sm ${pendingClass}`}
      >
        {/* profile button */}
        <button
          className="absolute -top-2 -right-2 text-[10px] px-1.5 py-0.5 rounded bg-blue-600 text-white"
          onClick={(e) => {
            e.stopPropagation();
            onOpenProfile(id);
          }}
